package org.auth.dataModel.model.entity.dictionary;
...
import org.auth.dataModel.model.entity.AuditSupport;
...
@Entity
@Table(name = "univer_dict", schema = EntityHelper.AUTH_SCHEMA)
@NamedQuery(name = UniverDictEntity.FIND_ALL, query = "SELECT ude FROM UniverDictEntity ude WHERE ude.sign=:sign")
@Cache(usage = CacheConcurrencyStrategy.TRANSACTIONAL)
@Audited
@AuditTable(value = "univer_dict_aud", schema = EntityHelper.AUTH_SCHEMA)
public class UniverDictEntity extends AbstractEntity<BigDecimal> implements AuditSupport {
	...
	@Id
	@SequenceGenerator(name = SEQ_NAME, sequenceName = "seq_univer_dict_id", schema = EntityHelper.AUTH_SCHEMA, allocationSize = 1)
